...
class CMaxyssWb{
    ...
    public static function PrepareItem($id = 0, $cabinet = 'DEFAULT'){
        if(intval($id) <= 0){
            ...
            $arSelect = Array("ID", "IBLOCK_ID", "NAME", "TAGS", $arSettings['BASE_PICTURE'], "PREVIEW_TEXT", "DETAIL_TEXT", "PROPERTY_PROP_MAXYSS_WB", "PROPERTY_PROP_MAXYSS_CARDID_WB", "PROPERTY_".$arSettings['DESCRIPTION'], "PROPERTY_CML2_ARTICLE", "PROPERTY_TYPE");// op
            ...
            $addin_card[GetMessage('WB_MAXYSS_DESCRIPTION')] = array(
                ...
                'params' => array(
                    array(
						...
						"value" => TruncateText($description, 997)// op
                    )
                )
            );
...
//            }
            ...
            $item = array(
                ...
                "card" => array(
                    "object" => $object,
                    "supplierVendorCode" => "",// op ($arSettings['ARTICLE']=='')? $arFields['ID'] : $arFields["PROPERTY_".strtoupper($arSettings['ARTICLE'])."_VALUE"],
...
//                                        ),
		...
		//file_put_contents("/home/bitrix/ext_www/tempusshop.ru/bitrix/modules/maxyss.wb/test.txt", print_r($item, true));
                    ...
                    if ($item_nmId > 0) {
                        if ($arFields["PROPERTY_" . strtoupper($iblock_shkod) . "_VALUE"] != '') {
							...
							if (intval($arProp[$arFields["ID"]]["PROP_MAXYSS_NMID_CREATED_WB"]["VALUE"]) > 0) {
								...
								// discounts
								if (intval($arProp[$arFields["ID"]]["PROP_MAXYSS_DISCOUNTS_WB"]["VALUE"]) > 0) {
									// ��������� ������
									$item_discounts[] = array(
										"discount" => intval($arProp[$arFields["ID"]]["PROP_MAXYSS_DISCOUNTS_WB"]["VALUE"]),
										"nm" => intval($arProp[$arFields["ID"]]["PROP_MAXYSS_NMID_CREATED_WB"]["VALUE"]),
									);
								} else {
									// ����� ������
									$item_discounts_revoke[] = intval($arProp[$arFields["ID"]]["PROP_MAXYSS_NMID_CREATED_WB"]["VALUE"]);
								}

								// promocodes
								if (intval($arProp[$arFields["ID"]]["PROP_MAXYSS_PROMOCODES_WB"]["VALUE"]) > 0) {
									// ��������� ����������
									$item_promocodes[] = array(
										"discount" => intval($arProp[$arFields["ID"]]["PROP_MAXYSS_PROMOCODES_WB"]["VALUE"]),
										"nm" => intval($arProp[$arFields["ID"]]["PROP_MAXYSS_NMID_CREATED_WB"]["VALUE"]),
									);
								} else {
									// ����� ����������
									$item_promocodes_revoke[] = intval($arProp[$arFields["ID"]]["PROP_MAXYSS_NMID_CREATED_WB"]["VALUE"]);
								}
...
//                                }
    ...
    public static function prepareAllItemsPrice($arSettings, $arrFilter = array(), $cabinet = "DEFAULT"){
        $cabinet =  $arSettings['LK'];
        $IBLOCK_ID = $arSettings["IBLOCK_ID"];
...
//file_put_contents("/home/bitrix/ext_www/tempusshop.ru/bitrix/modules/maxyss.wb/last_send.txt", print_r($arFilter, true));
        $res = CIBlockElement::GetList(Array("ID" => "ASC"), $arFilter, false, false, $arSelect);
        $arIds = array();
        while ($ob = $res->GetNextElement()) {
            $ar_tovar = array();
            ...
            if(!isset($arIds[$arFields['ID']])) {
                ...
                if ($ar_tovar["TYPE"] == 1) {
					...
					/*if($arFields["ID"] == 170427){
						$fff = "/home/bitrix/ext_www/tempusshop.ru/bitrix/modules/maxyss.wb/last_send.txt";
						//file_put_contents("/home/bitrix/ext_www/tempusshop.ru/bitrix/modules/maxyss.wb/last_send.txt", print_r($ar_tovar, true), FILE_APPEND);
						//file_put_contents("/home/bitrix/ext_www/tempusshop.ru/bitrix/modules/maxyss.wb/last_send.txt", print_r($arProps, true), FILE_APPEND);

						file_put_contents($fff, "key_cabinet - " . $key_cabinet, FILE_APPEND);
						file_put_contents($fff, "cabinet - " . $cabinet, FILE_APPEND);
						file_put_contents($fff, "item_nmId2 - " . $item_nmId2, FILE_APPEND);
						file_put_contents($fff, "item_nmId - " . $item_nmId, FILE_APPEND);
...
$Logger = new TsLogger("/wb/uploadAllStocks/");
        $arSettings = self::settings_wb($cabinet);
        $Authorization = $arSettings['AUTHORIZATION'];

        $items = self::prepareAllItemsStock($arSettings, $arrFilter, $cabinet);
...
$Logger->log("LOG", "stocks - ".print_r($items["stocks"], true));
    ...
    public static function updateStock($Authorization, $items, $cabinet){
        ...
        $bck = self::bck_wb();
		$Logger = new TsLogger("/wb/uploadAllStocks/");
		file_put_contents("/home/bitrix/logs/wb/uploadAllStocks/last_updateStock.txt", print_r($items, true));
        if($bck['BCK'] && $bck['BCK'] != "Y") {
            if (!empty($items)) {
							...
							if(is_array($arResult['error']["data"]) && $arResult['error']["code"] == "NotFound"){
                                foreach ($arResult['error']["data"] as $data){
									if(strlen($data['sku']) > 10){
										file_put_contents("/home/bitrix/logs/wb/updateStock_error.txt", $data['sku'] . "\r\n", FILE_APPEND);
										$strSql = "SELECT * FROM b_iblock_element_prop_s16 WHERE PROPERTY_2821 = '{$data["sku"]}'";
										$results = $DB->Query($strSql, false, $err_mess.__LINE__);
										if ($row = $results->Fetch()){
											file_put_contents("/home/bitrix/logs/wb/updateStock_error_all.txt", print_r($row, true) . "\r\n", FILE_APPEND);
                ...
                if(!empty($res['orders'])) {
                    foreach ($res['orders'] as $order_wb) {
						...
						if (intval($order_wb['id']) > 0 && intval($order_wb["chrtId"]) > 0) {
							file_put_contents("/home/bitrix/logs/wb/loadNewOrders.txt", "ищем заказ - {$unq} - " . date('Y-m-d H:i:s') . " - " . $order_wb['id'] . "\r\n", FILE_APPEND);
							$id_bx_order = self::getOrder($order_wb['id']);
							file_put_contents("/home/bitrix/logs/wb/loadNewOrders.txt", "нашли заказ - {$unq} - " . date('Y-m-d H:i:s') . " - " . $order_wb['id'] . " id_bx_order - " . $id_bx_order . "\r\n", FILE_APPEND);
							if ($id_bx_order == 0) {
								$id_bx_order_new = self::createOrder($order_wb, $order_user, $arSettings, $cabinet);
								file_put_contents("/home/bitrix/logs/wb/loadNewOrders.txt", "Конец Создаем заказ - {$unq} - " . date('Y-m-d H:i:s') . " - " . $order_wb['id'] . " id_bx_order_new - " . $id_bx_order_new . "\r\n", FILE_APPEND);
							}
						}
					}
				}
...
//                                break;
                    ...
                    {
                        file_put_contents($_SERVER['DOCUMENT_ROOT']."/bitrix/modules/".MAXYSS_WB_NAME."/log_order_error.txt", date("Y-m-d H:i:s") . " - " . print_r($order_wb, true).PHP_EOL, FILE_APPEND);
                        file_put_contents($_SERVER['DOCUMENT_ROOT']."/bitrix/modules/".MAXYSS_WB_NAME."/log_order_error.txt", date("Y-m-d H:i:s") . " - " . print_r($result->getErrorMessages(), true).PHP_EOL, FILE_APPEND);
                    }
...
//                $flag_button = true;
	...
	public static function getAllItemsAvail($arSettings){
        ...
        // убираем товары которые в карантине
		...
		if(CModule::IncludeModule("iblock")){
			...
			while($ar = $rs->GetNext()){
				...
				if(count($arExcluded) > 0){
					...
					if($key_cabinet !== false){
						if(in_array($ar["PROPERTY_PROP_MAXYSS_NMID_CREATED_WB_VALUE"][$key_cabinet], $arExcluded)){
							continue;
						}
					}
				}
	...
	public static function getPropsSync($cabinet = "DEFAULT"){
		...
		$res = $DB->Query("SELECT * FROM b_iblock_element_prop_m16 WHERE IBLOCK_PROPERTY_ID IN ('" . (implode("','", array_keys($arProp))) . "') AND DESCRIPTION = '{$cabinet}'");
		while ($ar = $res->Fetch()){
			if($ar["VALUE"]){
	...
	public static function getItemsWB(){
		...
		global $DB;
		if(!CModule::IncludeModule("main") || !CModule::IncludeModule("iblock") || !CModule::IncludeModule("currency") || !CModule::IncludeModule("catalog") || !CModule::IncludeModule("panel_manager")) return;

		$objPricelist = new CPanelPricelist;
		$price = $objPricelist->getPriceByFilter(array("price_id" => "wb"), "bitrix_id", array("model","bitrix_id",));

		$arSettings = self::settings_wb("WR");

		foreach(self::getAllItemsAvail($arSettings) as $key => $arItem){
			$itemsAvail[$arItem["ID"]] = $arItem;
		}

		foreach($price as $key => $arItem){
			if(!$itemsAvail[$arItem["bitrix_id"]]){
				unset($price[$key]);
			}
		}
