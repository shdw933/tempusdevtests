...
//            }
            ...
            $item = array(
                ...
                "card" => array(
                    ...
                    "nomenclatures" => array(
                        array( // ������ ����������� ������.
                            "vendorCode" => ($arSettings['ARTICLE']=='')? $arFields['ID'] : $arFields["PROPERTY_".strtoupper($arSettings['ARTICLE'])."_VALUE"].$article_dop, // ������� ������.
                            "variations" => array(
                                array( // ������ �������� ������. ���� ���� - ���� ��������.
                                    "barcode" => $arFields["PROPERTY_".strtoupper($arSettings['SHKOD'])."_VALUE"], // �������� ������.
    ...
    public static function  setStatusV3($order_wb = array(), $id_bx_order = 0, $status_bx, $arSettings){
        ...
        if($id_bx_order > 0 && !empty($order_wb)){
            ...
            switch ($order_wb['supplierStatus'].$order_wb['wbStatus']){
                case 'new'.'waiting':
//                    echo '<pre>', print_r("�����"), '</pre>' ; // 0
                    $status_result = 0;
                    break;
                case 'confirm'.'waiting':
//                    echo '<pre>', print_r("�� ������"), '</pre>' ;//  2  1 - ������ �����
...
//                    echo '<pre>', print_r("������� ������"), '</pre>' ; // 6
                    $status_result = 6;
                    break;
                case 'complete'.$order_wb['wbStatus']:
//                    echo '<pre>', print_r("������� �� ���"), '</pre>' ; // 5
                    $status_result = 5;
                    break;
                case $order_wb['supplierStatus'].'canceled':
//                    echo '<pre>', print_r("�� �������� / �������"), '</pre>' ; // 1  3 - ��������� ������� ���������
                    if(!$arSettings['TRIGGERS']['CANCEL_TRIGER']) {
                        $status_result = 1;
                    }
                    else $flag_status_break = true;
            ...
            if ($status_result != '' && $arSettings["STATUS_BY"][$status_result] && $arSettings["STATUS_BY"][$status_result] != $status_bx) {
                ...
                {
                    $eventLog = new \CEventLog;
                    $eventLog->Add(array("SEVERITY" => 'INFO', "AUDIT_TYPE_ID" => 'change_st�tus_order', "MODULE_ID" => MAXYSS_WB_NAME, "ITEM_ID" => $order->getId(), "DESCRIPTION" => serialize($result->getErrors())));
                }
            ...
            if ($arOrder = $rsOrders->Fetch())
            {
                ...
                while($prop = $obProps->Fetch()){
                    if($prop['CODE'] == "MAXYSS_WB_NUMBER") {
                        ...
                        if(!file_exists($FPPath))  $arWbOrders = array(intval($prop['VALUE']));  /// ���� ���-�� �� ��������� � ���� �� ���������
                    }
                }
//                $flag_button = true;
            }
