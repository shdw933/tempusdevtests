...
if(Loader::includeModule('sale') && Loader::includeModule('iblock') && CModule::IncludeModule(MAXYSS_MODULE_NAME) && ($GLOBALS['APPLICATION']->GetGroupRight(MAXYSS_MODULE_NAME) >= "R")) {
    if(!VERSION_OZON_2){
        ...
        if($iblock_id) {
            if ($_REQUEST['save_custom_cat'] && !empty($_REQUEST['cat_ids']) && ($GLOBALS['APPLICATION']->GetGroupRight(MAXYSS_MODULE_NAME) == "W")) {
    ...
    <!--                                <form action="" name="save_cat">-->
                                        ...
                                        if (_is_curl_installed()) {
                                            ...
                                            $arCategorys = CRestQuery::rest_query($ClientId, $ApiKey, $base_url = OZON_BASE_URL, $data_string, "/v2/category/tree");
                                            $sortCategorys = array();
                                            if(!empty($arCategorys)&& !isset($arCategorys['error'])){
                                                foreach ($arCategorys as &$sort) {
                                                    usort($sort['children'], "sort_cat");
                                                    foreach ($sort['children'] as &$item_cat){
                                                        usort($item_cat['children'], "sort_cat");
                                                    }
                                        ...
                                        if(!empty($sortCategorys)) {
                                            ...
                                            if (!function_exists('array_write')) {
                                                ...
                                                function array_write($array, $i, $arSections)
                                                {
                                                    foreach ($array as $key => $value) {
                                                        ...
                                                        if (count($value['children']) > 0) {
                                                            ...
                                                            <div style="padding-left: 60px">
                                                                <input type="checkbox" name="cat_ids[]" <?echo (is_array($arSections) && in_array($value['category_id'], $arSections))? 'checked' : ''?> id="id_<?=$value['category_id']?>" value="<?=$value['category_id'].'/'.$value['title']?>"><label for="id_<?=$value['category_id']?>"><?=$value['title'].' ('.$value['category_id'].')'?></label>
                                                            </div>
                                                            <?
                                                        }
                                                    }
    ...
    <!--                                </form>-->
                    ...
                    <?if(!empty($sortCategorys) && $GLOBALS['APPLICATION']->GetGroupRight(MAXYSS_MODULE_NAME) == "W") {?>
                    <tr class="heading">
