...
$mid = MAXYSS_MODULE_NAME;
global $APPLICATION;
IncludeModuleLangFile(__FILE__);
IncludeModuleLangFile($_SERVER["DOCUMENT_ROOT"].BX_ROOT."/modules/main/admin/group_rights.php");
...
if(Loader::includeModule('catalog') && Loader::includeModule('iblock') && CModule::IncludeModule(MAXYSS_MODULE_NAME) && ($GLOBALS['APPLICATION']->GetGroupRight(MAXYSS_MODULE_NAME) >= "R")){

    if(($_REQUEST['save'] || $_REQUEST['apply']) && check_bitrix_sessid() && $APPLICATION->GetGroupRight(MAXYSS_MODULE_NAME)=='W'){
...
//        }
        ...
        CMaxyssOzon:: saveOptions('weight_min');
        CMaxyssOzon:: saveOptions('weight_max');
...
//            "OPTIONS" => $arBackParametrs,
    ...
    $arTabs[$key_rights-1] = array("DIV" => "edit".$key_rights, "TAB" => GetMessage("MAXYSS_OZON_RIGHT"), "TITLE" => GetMessage("MAXYSS_OZON_RIGHT"),"ICON" => "settings","PAGE_TYPE" => "site_settings");
    ...
    <?}else{?>
        <style>
            ...
            <form class="maxyss_wrap" action="<?echo $APPLICATION->GetCurPage()?>?mid=<?=htmlspecialchars($mid)?>&amp;lang=<?echo LANG?>" method="post">
<!--            <form class="maxyss_wrap" action="--><?//=MAXYSS_MODULE_NAME?><!--_ozon_maxyss_general.php?lang=--><?//=LANGUAGE_ID?><!--" method="post">-->
                ...
                foreach($arTabs as $key => $arTab)
                {
                    ...
                    {
                        $tabControl->BeginNextTab();
                        $arOptions = CMaxyssOzon::getOptions($arTab["SITE_ID"], array('OZON_ID', 'OZON_API_KEY', "SERVER_NAME", "DELIVERY_SERVICE_V3", "SKLAD_ID_V3", "DEACTIVATE", "OZON_V3", "LIMIT", "LIMIT_PRICE", "WEIGHT_MIN", "WEIGHT_MAX"));
                        ?>
                        ...
                        <tr>
                            ...
                            <td class="adm-detail-content-cell-r">
                                <input name="ozon_id[<?= $arTab["SITE_ID"] ?>]" value="<?
                                echo(Option::getRealValue(MAXYSS_MODULE_NAME, "OZON_ID", $arTab["SITE_ID"])) ?>">
                            </td>
                        </tr>
                        <tr>
                            <td class="adm-detail-content-cell-l"><?= GetMessage('MAXYSS_OZON_OZON_API_KEY') ?></td>
                            <td class="adm-detail-content-cell-r">
                                <input name="ozon_api_key[<?= $arTab["SITE_ID"] ?>]" value="<?
                                echo(Option::getRealValue(MAXYSS_MODULE_NAME, "OZON_API_KEY", $arTab["SITE_ID"])) ?>">
                            </td>
                        </tr>
                        <?/*?><tr>
                        ...
                        ?>
                        <?
                        if (Option::getRealValue(MAXYSS_MODULE_NAME, "OZON_ID", $arTab["SITE_ID"]) != '') {
                            $warehouses = array();
                            ...
                            if (!$warehouses['error'] && !empty($warehouses)) {
                                ...
                                $arLimitWarehousesWeightMin = unserialize($arOptions[$arTab["SITE_ID"]]["WEIGHT_MIN"]);
                                $arLimitWarehousesWeightMax = unserialize($arOptions[$arTab["SITE_ID"]]["WEIGHT_MAX"]);
                                ...
                                foreach ($warehouses as $warehouse) {
                                    ...
                                    if (is_array($arSklads)) {
                                        foreach ($arSklads as $sklad) {
                                            $selected = '';
                                            if (is_array($sklad_id[$warehouse["warehouse_id"]]) && in_array($sklad['ID'], $sklad_id[$warehouse["warehouse_id"]]))
                                                $selected = 'selected = "selected"';
                                    ...
                                    <tr>
                                        <td colspan="2"
                                            style="font-weight: 600; text-align: center"><?= $warehouse["name"] ?></td>
                                    </tr>
                                    <tr>
                                        <td style=" text-align: center">
                                            <?= GetMessage('MAXYSS_OZON_V3_DEACTIVATE_WAREHOUSE') ?>
                                            <input type="hidden"
                                                   name="deactivate[<?= $arTab["SITE_ID"] ?>][<?= $warehouse["warehouse_id"] ?>]"
                                                   value="N">
                                            <input id="<?= $warehouse["warehouse_id"] . $arTab["SITE_ID"] ?>"
                                                   type="checkbox"
                                                   name="deactivate[<?= $arTab["SITE_ID"] ?>][<?= $warehouse["warehouse_id"] ?>]" <?
                                            echo ($arDeactivateWarehouses[$warehouse["warehouse_id"]] == 'Y') ? 'checked = "checked"' : '' ?>
                                                   value="Y">
                                        </td>
                                        <td style=" text-align: right">
                                            <?= GetMessage('MAXYSS_OZON_V3_LIMIT_NULL') ?><span
                                                    data-hint="<?= GetMessage('MAXYSS_OZON_V3_LIMIT_NULL_TIP') ?>"></span>
                                            <?= GetMessage('MAXYSS_OZON_V3_LIMIT_WAREHOUSE') ?>
                                            <input class="limits_ms" id="<?= $warehouse["warehouse_id"] ?>"
                                                   type="number"
                                                   name="limit[<?= $arTab["SITE_ID"] ?>][<?= $warehouse["warehouse_id"] ?>]"
                                                   value="<?
                                                   echo $arLimitWarehouses[$warehouse["warehouse_id"]]; ?>"><span
                                                    data-hint="<?= GetMessage('MAXYSS_OZON_V3_LIMIT_WAREHOUSE_TIP') ?>"></span>

                                            <span style="display: inline-block; width: 15px"></span>
                                            <?= GetMessage('MAXYSS_OZON_V3_LIMIT_WAREHOUSE_PRICE') ?>
                                            <input class="limits_ms" id="<?= $warehouse["warehouse_id"] ?>"
                                                   type="number"
                                                   name="limit_price[<?= $arTab["SITE_ID"] ?>][<?= $warehouse["warehouse_id"] ?>]"
                                                   value="<?
                                                   echo $arLimitWarehousesPrice[$warehouse["warehouse_id"]]; ?>"><span
                                                    data-hint="<?= GetMessage('MAXYSS_OZON_V3_LIMIT_WAREHOUSE_PRICE_TIP') ?>"></span>
                                    ...
                                    <tr>
                                        <td></td>
                                        <td style=" text-align: right">
                                            <?= GetMessage('MAXYSS_OZON_V3_LIMIT_WAREHOUSE_WEIGHT_MIN') ?>
                                            <input class="limits_ms" step=1 id="<?= $warehouse["weight_min_id"] ?>"
                                                   type="number"
                                                   name="weight_min[<?= $arTab["SITE_ID"] ?>][<?= $warehouse["warehouse_id"] ?>]"
                                                   value="<?
                                                   echo $arLimitWarehousesWeightMin[$warehouse["warehouse_id"]]; ?>"><?
                                            echo GetMessage('MAXYSS_OZON_WEIGHT_UNIT'); ?>
                                            <?= GetMessage('MAXYSS_OZON_V3_LIMIT_WAREHOUSE_WEIGHT_MAX') ?>
                                            <input class="limits_ms" step=1 id="<?= $warehouse["weight_max_id"] ?>"
                                                   type="number"
                                                   name="weight_max[<?= $arTab["SITE_ID"] ?>][<?= $warehouse["warehouse_id"] ?>]"
                                                   value="<?
                                                   echo $arLimitWarehousesWeightMax[$warehouse["warehouse_id"]]; ?>"><?
                                            echo GetMessage('MAXYSS_OZON_WEIGHT_UNIT'); ?><span
                                                    data-hint="<?= GetMessage('MAXYSS_OZON_V3_LIMIT_WAREHOUSE_WEIGHT_TIP') ?>"></span>
                                        </td>
                                    </tr>


                                    <tr style="background-color: #e0e8ea">
                                        <td style="text-align: center"><?= GetMessage("MAXYSS_OZON_SKLAD_TEXT") ?></td>
                                        <td style="text-align: center"><?= GetMessage("MAXYSS_OZON_DELYVERY_TEXT") ?></td>
                                    </tr>
                                    <tr>
                                        <td class="adm-detail-content-cell-l">
                                            <input type="hidden"
                                                   name="sklad_id_v3[<?= $arTab["SITE_ID"] ?>][<?= $warehouse["warehouse_id"] ?>]"
                                                   value="">
                                            <select multiple
                                                    name="sklad_id_v3[<?= $arTab["SITE_ID"] ?>][<?= $warehouse["warehouse_id"] ?>][]">
                                                <?
                                                            ...
                                                            <td class="adm-detail-content-cell-r">
                                                                <select name="delivery_service_v3[<?= $arTab["SITE_ID"] ?>][<?= $warehouse["warehouse_id"] ?>][<?= $d_metod['id'] ?>]">
                                                                    ...
                                                                    foreach ($arDeliverySelect as $key_type => $type) {
                                                                        ...
                                                                        echo ($arDeliverysMetod[$warehouse["warehouse_id"]][$d_metod['id']] == $key_type) ? 'selected = "selected"' : '' ?>><?= $type ?></option>
                                                                        <?
                                                                    } ?>
                                                                </select>
                                                            </td>
                        ...
                        <tr>
                            ...
                            <td class="adm-detail-content-cell-r">
                                <input type="checkbox" name="ozon_requared" id="ozon_requared_<?= $arTab["SITE_ID"] ?>"
                                       class="" <?
                                echo (Option::get(MAXYSS_MODULE_NAME, "REQURED_MORE", "") == 'Y') ? 'checked = "checked"' : '' ?>>
                                <!--                                        <label class="adm-designed-checkbox-label" for="ozon_requared_-->
                                <?//=$arTab["SITE_ID"]
                                ?><!--" title=""></label>-->
                            </td>
                        </tr>
                        ...
                        <tr>
                            <td width="40%"
                                class="adm-detail-content-cell-l"><?= GetMessage('MAXYSS_OZON_DOMEN_NAME') ?></td>
                            <td width="60%" class="adm-detail-content-cell-r">
                                <input type="text" name="server_name[<?= $arTab["SITE_ID"] ?>]" value="<?
                                echo(Option::getRealValue(MAXYSS_MODULE_NAME, "SERVER_NAME", $arTab["SITE_ID"])) ?>"
                                       size="50"> <input type="button"
                                                         onclick="this.form['server_name[<?= $arTab["SITE_ID"] ?>]'].value = window.location.host;"
                                                         value="<?= GetMessage('MAXYSS_OZON_DOMEN_NAME_REAL') ?>">
                            </td>
                        </tr>
                        <div id="ans"></div>
                        <?
