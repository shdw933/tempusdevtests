...
if(Loader::includeModule('catalog') && Loader::includeModule('iblock') && Loader::includeModule(MAXYSS_MODULE_NAME) && ($GLOBALS['APPLICATION']->GetGroupRight(MAXYSS_MODULE_NAME) >= "R")){
    $APPLICATION->AddHeadScript("/bitrix/tools/maxyss.ozon/filter_conditions/script.js");

    if(($_REQUEST['save'] || $_REQUEST['apply']) && ($GLOBALS['APPLICATION']->GetGroupRight(MAXYSS_MODULE_NAME) == "W")){
    CMaxyssOzon:: saveOptions('active_on');
    CMaxyssOzon:: saveOptions('active_fid');
    CMaxyssOzon:: saveOptions('no_upload_product');
    CMaxyssOzon:: saveOptions('re_upload_product');
...
//    CMaxyssOzon:: saveOptions('price_type_premium_formula_action');
    ...
    CMaxyssOzon:: saveOptions('price_type_min');
    CMaxyssOzon:: saveOptions('price_prop_min');
    CMaxyssOzon:: saveOptions('price_type_min_prop');
    CMaxyssOzon:: saveOptions('price_type_min_no_discount');
    CMaxyssOzon:: saveOptions('price_type_min_formula');
    CMaxyssOzon:: saveOptions('price_type_min_formula_action');
    CMaxyssOzon:: saveOptions('VAT_CUSTOM');
...
//
    ...
    $arActive = CMaxyssOzon::getOptions(false, array('ACTIVE_ON', 'PERIOD', 'RE_UPLOAD_PRODUCT','ACTIVE_FID'));
    ...
    foreach ($arActive as $key=>$active){
        if($active['ACTIVE_ON'] == 'Y'){
            ...
            if($arRes = $res->GetNext()) {
                ...
                {
                    if($arRes['AGENT_INTERVAL'] != $active['PERIOD']) {
                        if ($active['RE_UPLOAD_PRODUCT'] == 'Y') {
                            CAgent::Delete(intval($arRes['ID']));
                            CAgent::AddAgent(
                    ...
                    elseif ($arRes['AGENT_INTERVAL'] == $active['PERIOD'] && $active['RE_UPLOAD_PRODUCT'] == 'Y')
                    {
                        CAgent::Delete(intval($arRes['ID']));
                        CAgent::AddAgent(
                            "CMaxyssOzonAgent::OzonUploadProduct('" . $key . "',1);",
        ...
        // fid
        if($active['ACTIVE_FID'] == 'Y'){
            $res_fid = CAgent::GetList(Array("ID" => "DESC"), array("NAME" => "CMaxyssOzonAgent::OzonFid('".$key."%"));
            if($arResFid = $res_fid->GetNext()) {

            }
            else
            {
                CAgent::AddAgent(
                    "CMaxyssOzonAgent::OzonFid('".$key."');",
                    "maxyss.ozon",
                    "N",
                    '3600',
                    "",
...
//            "OPTIONS" => $arBackParametrs,
    ...
    <form action="<?=MAXYSS_MODULE_NAME?>_ozon_maxyss.php?lang=<?=LANGUAGE_ID?>" method="post">
        ...
        foreach($arTabs as $key => $arTab)
        {
            ...
            $iblock_price_min_select = '<option value=""></option>';
            if(intval($iblock_id)>0)
            {
                $iblock_art = '';
                ...
                $iblock_price_min = '';
                $iblock_price_min = $arOptions[$arTab["SITE_ID"]]["PRICE_TYPE_MIN_PROP"];

                $iblock_prop_ozon_id = '<option value=""></option>';

//                $res = CIBlock::GetProperties(intval($iblock_id), Array('name'=>'asc'), Array("MULTIPLE"	=> "N", "PROPERTY_TYPE" => array("S","N")));

                $arProps = \Bitrix\Iblock\PropertyTable::getList(array(
                    'select' => array('*'),
                    'filter' => array('IBLOCK_ID' => intval($iblock_id), Array("MULTIPLE"=> "N", "PROPERTY_TYPE" => array("S","N")))
                ))->fetchAll();
                foreach ($arProps as $res_arr )
                {
                    ...
                    $selected_min_price = '';
                    if($iblock_price_min !='' && $iblock_price_min == $res_arr['CODE'])
                        $selected_min_price = 'selected = "selected"';
                    $iblock_price_min_select .= '<option '.$selected_min_price.' value="'.$res_arr['CODE'].'">'.'['.$res_arr['ID'].'] '.$res_arr['NAME'].'</option>';
                }
            }
            ...
            $price_type_min = '';
            $price_type_min = $arOptions[$arTab["SITE_ID"]]["PRICE_TYPE_MIN"];
            ...
            $price_type_min_select = '<option value=""></option>';
            ...
            while ($arPriceType = $dbPriceType->Fetch())
            {
                ...
                $selected = '';
                if($price_type_min == $arPriceType['ID'])
                    $selected = 'selected = "selected"';
                $price_type_min_select .= '<option '.$selected.' value="'.$arPriceType['ID'].'">'.'['.$arPriceType['ID'].'] '.$arPriceType['NAME_LANG'].'</option>';
            }
            ...
            if($arOptions[$arTab["SITE_ID"]]["SKLAD_ID"] == ''){
                ...
                {
                    $selected = '';
                    if(is_array($sklad_id) && in_array($arSklad['ID'], $sklad_id))
                        $selected = 'selected = "selected"';
...
<!--            <tbody>-->
            ...
            <tr>
                <td class="adm-detail-content-cell-l"><?=GetMessage('MAXYSS_OZON_FID_ACTIVE')?></td>
                <td class="adm-detail-content-cell-r">
                    <input type="hidden" name="active_fid[<?=$arTab["SITE_ID"]?>]" value="N">
                    <input type="checkbox" name="active_fid[<?=$arTab["SITE_ID"]?>]" id="active_fid_<?=$arTab["SITE_ID"]?>" class="adm-designed-checkbox" <?echo  ($arOptions[$arTab["SITE_ID"]]["ACTIVE_FID"] == 'Y')? 'checked = "checked"' : ''?> value="Y">
                    <label class="adm-designed-checkbox-label" for="active_fid_<?=$arTab["SITE_ID"]?>" title=""></label>
                    <?
                    if(file_exists($_SERVER['DOCUMENT_ROOT']."/bitrix/catalog_export/" . MAXYSS_MODULE_NAME ."/". $arTab["SITE_ID"]. "_fid.php")){
                        ?> <?echo GetMessage('MAXYSS_OZON_FID_LINK');?><a target="_blank" href="/bitrix/catalog_export/maxyss.ozon/<?=$arTab["SITE_ID"]?>_fid.php"><?=$arTab["SITE_ID"]?>_fid.php</a><?
                    }
                    ?>
                </td>
            </tr>
            ...
            <?
            if(!isset($arOptions[$arTab["SITE_ID"]]["VAT_CUSTOM"]) || $arOptions[$arTab["SITE_ID"]]["VAT_CUSTOM"] == 0)
                $select_vat = '<option selected = "selected" value="0">'.GetMessage('MAXYSS_OZON_VAT_NO_CUSTOM').'</option>';
            else
                $select_vat = '<option value="0">'.GetMessage('MAXYSS_OZON_VAT_NO_CUSTOM').'</option>';

            $dbVat = CCatalogVat::GetListEx(  array('sort'=>'asc'), array('ACTIVE'=>'Y')  );
            $custom_vat = $arOptions[$arTab["SITE_ID"]]["VAT_CUSTOM"];
            while($arVat = $dbVat->GetNext()) {
                $selected_vat = '';
                if($arOptions[$arTab["SITE_ID"]]["VAT_CUSTOM"] == $arVat["ID"]) $selected_vat = 'selected = "selected"';
                $select_vat .= '<option '.$selected_vat.' value="'.$arVat["ID"].'">'.$arVat["NAME"].'</option>';
            }
            ?>
            <tr>
                <td class="adm-detail-content-cell-l"><?=GetMessage('MAXYSS_OZON_VAT_CUSTOM')?></td>
                <td class="adm-detail-content-cell-r">
                    <select name="VAT_CUSTOM[<?=$arTab["SITE_ID"]?>]" id="VAT_CUSTOM<?=$arTab["SITE_ID"]?>">
                        <?echo $select_vat?>
                    </select>
                </td>
            </tr>
            ...
            <tr>
                <td class="adm-detail-content-cell-l"><?=GetMessage('MAXYSS_OZON_AGENT_REUPLOAD_PRODUCT')?></td>
                <td class="adm-detail-content-cell-r">
                    <input type="hidden" name="re_upload_product[<?=$arTab["SITE_ID"]?>]" value="N">
                    <input type="checkbox" name="re_upload_product[<?=$arTab["SITE_ID"]?>]" id="re_upload_product[<?=$arTab["SITE_ID"]?>]" class="adm-designed-checkbox" <?echo  ($arOptions[$arTab["SITE_ID"]]["RE_UPLOAD_PRODUCT"] == 'Y')? 'checked = "checked"' : ''?> value="Y">
                    <label class="adm-designed-checkbox-label" for="re_upload_product[<?=$arTab["SITE_ID"]?>]" title=""></label><span data-hint="<?=GetMessage('MAXYSS_OZON_AGENT_REUPLOAD_PRODUCT_TIP')?>"></span>
                </td>
            </tr>
...
<!--            </tr>-->
            <tr>
                ...
                <tr>
                    <td class="adm-detail-content-cell-l"><?=GetMessage('MAXYSS_OZON_PRICE_MIN')?></td>
                    <td class="adm-detail-content-cell-r">
                        <table>
                            <tr>
                                <td>
                                ...
                                <td>
                                    <input type="hidden" name="price_prop_min[<?=$arTab["SITE_ID"]?>]" value="">
                                    <input type="checkbox" name="price_prop_min[<?=$arTab["SITE_ID"]?>]" id="price_prop_min_<?=$arTab["SITE_ID"]?>" class="adm-designed-checkbox price_to_prop" <?echo ($arOptions[$arTab["SITE_ID"]]["PRICE_PROP_MIN"]=="Y")? 'checked' : '' ;?> value="Y">
                                    <label class="adm-designed-checkbox-label" for="price_prop_min_<?=$arTab["SITE_ID"]?>" title=""></label>
                                </td>
                                <td <?echo ($arOptions[$arTab["SITE_ID"]]["PRICE_PROP_MIN"]=="Y")? '' : 'style="display: none"' ;?>>
                                    <select name="price_type_min_prop[<?=$arTab["SITE_ID"]?>]">
                                        <?echo $iblock_price_min_select?>
                                    </select>
                                </td>
                                <td <?echo ($arOptions[$arTab["SITE_ID"]]["PRICE_PROP_MIN"]=="Y")? 'style="display: none"' : '' ;?>>
                                    <select name="price_type_min[<?=$arTab["SITE_ID"]?>]">
                                        <?echo $price_type_min_select?>
                                    </select>
                                </td>
                                <td <?echo ($arOptions[$arTab["SITE_ID"]]["PRICE_PROP_MIN"]=="Y")? 'style="display: none"' : '' ;?>>
                                    <input type="hidden" name="price_type_min_no_discount[<?=$arTab["SITE_ID"]?>]" value="">
                                    <input type="checkbox" name="price_type_min_no_discount[<?=$arTab["SITE_ID"]?>]" <?echo ($arOptions[$arTab["SITE_ID"]]["PRICE_TYPE_MIN_NO_DISCOUNT"]=="Y")? 'checked' : '' ;?> id="price_type_min_no_discount_old_<?=$arTab["SITE_ID"]?>" class="adm-designed-checkbox"  value="Y">
                                    <label class="adm-designed-checkbox-label" for="price_type_min_no_discount_old_<?=$arTab["SITE_ID"]?>" title=""></label>
                                </td>
                                <td <?echo ($arOptions[$arTab["SITE_ID"]]["PRICE_PROP_MIN"]=="Y")? 'style="display: none"' : '' ;?>><?echo GetMessage('MAXYSS_OZON_PRICE_WITHOUT_DISCOUNT')?></td>
                                <td>
                                    <select name="price_type_min_formula_action[<?=$arTab["SITE_ID"]?>]">
                                        <option value="NOT" <?echo ($arOptions[$arTab["SITE_ID"]]["PRICE_TYPE_MIN_FORMULA_ACTION"] == 'NOT')? 'selected' : '';?>><?=GetMessage('MAXYSS_OZON_PRICE_TYPE_FORMULA_ACTION_NOT')?></option>
                                        <option value="MULTIPLY" <?echo ($arOptions[$arTab["SITE_ID"]]["PRICE_TYPE_MIN_FORMULA_ACTION"] == 'MULTIPLY')? 'selected' : '';?>><?=GetMessage('MAXYSS_OZON_PRICE_TYPE_FORMULA_ACTION_MULTIPLY')?></option>
                                        <option value="DIVIDE" <?echo ($arOptions[$arTab["SITE_ID"]]["PRICE_TYPE_MIN_FORMULA_ACTION"] == 'DIVIDE')? 'selected' : '';?>><?=GetMessage('MAXYSS_OZON_PRICE_TYPE_FORMULA_ACTION_DIVIDE')?></option>
                                        <option value="ADD" <?echo ($arOptions[$arTab["SITE_ID"]]["PRICE_TYPE_MIN_FORMULA_ACTION"] == 'ADD')? 'selected' : '';?>><?=GetMessage('MAXYSS_OZON_PRICE_TYPE_FORMULA_ACTION_ADD')?></option>
                                        <option value="SUBTRACT" <?echo ($arOptions[$arTab["SITE_ID"]]["PRICE_TYPE_MIN_FORMULA_ACTION"] == 'SUBTRACT')? 'selected' : '';?>><?=GetMessage('MAXYSS_OZON_PRICE_TYPE_FORMULA_ACTION_SUBTRACT')?></option>
                                    </select>
                                    <input type="text" name="price_type_min_formula[<?=$arTab["SITE_ID"]?>]" value="<?echo $arOptions[$arTab["SITE_ID"]]["PRICE_TYPE_MIN_FORMULA"];?>">
                                </td>
                            </tr>
                        </table>
                    </td>
...
<script type="text/javascript">
    ...
    function property_update(iblock_id, div, sid){
        ...
        $.ajax({
            ...
            success: function(data) {
                ...
                tab.find($('[name*="description"]')).empty().html('<option value="DETAIL_TEXT" ><?=GetMessage('MAXYSS_OZON_ANONS_DETAIL')?></option><option value="PREVIEW_TEXT" ><?=GetMessage('MAXYSS_OZON_ANONS_ANONS')?></option><optgroup label="��������">' + data + '</optgroup>');
                ...
                tab.find($('[name*="price_type_min_prop"]')).empty().html('<option value=""></option>'+data);
                },
            error:  function(xhr, str){alert("<?=GetMessage('MAXYSS_OZON_MODULE_ERROR_AJAX')?>: " + xhr.responseCode);}
        });
        $.ajax({
    ...
    function downLoadId(lid) {
        ...
        let btn_close = {
            title: "<?echo GetMessage('MAXYSS_OZON_DOWNLOAD_ID_OZON_CLOSE');?>",
            id: 'OZON_CLOSE',
            name: 'OZON_CLOSE',
        ...
        let btn_save = {
            title: "<?echo GetMessage('MAXYSS_OZON_DOWNLOAD_ID_OZON_RUN');?>",
            id: 'OZON_RUN',
            name: 'OZON_RUN',
            ...
            action: function () {
                ...
                if(prop !=='') {
                    ...
                    alert("<?echo GetMessage('MAXYSS_OZON_DOWNLOAD_MESSAGE_FAIL');?>");
                }

            }
        };
        if(prop !==''){
            message_download = "<?echo GetMessage('MAXYSS_OZON_DOWNLOAD_MESSAGE_OK')?>";
        }
        else{
            message_download = "<?echo GetMessage('MAXYSS_OZON_DOWNLOAD_MESSAGE_FAIL')?>";
        }
        DialogLk = new BX.CDialog({
            title: "<?echo GetMessage('MAXYSS_OZON_DOWNLOAD_ID_OZON_TITLE_POPUP')?>",
                ...
                if (IS_JSON) {
                    if(obj.go_run){
                        ...
                        if(obj.mess === 'end'){
                            ...
                            $('#download_answer').parent().append( "<br><strong>" + "<?echo GetMessage('MAXYSS_OZON_DOWNLOAD_MESSAGE_SUCCESS')?>"+"</strong>" );
                    ...
                    else
                    {
                        if(obj.mess) {
                            ...
                            if (obj.mess === 'end')
                                ...
                                $('#download_answer').parent().append("<br><strong>" + obj.mess + "</strong>");
                        }
                    }
                } else {
                    alert('not valid json');
