...
//        echo '<pre>', print_r($str_result), '</pre>' ;
    ...
    public static function rest_supplies_add($base_url = WB_BASE_URL, $data_string, $path, $Authorization = false){
        ...
        if ($str_result->info->http_code == 201 && strlen($str_result->response)>0) {
            ...
            // Ќекорректный статус поставки
            $res = array('error' => '”же есть активна€ поставка');
        }else{
            // ¬нутренн€€ ошибка сервера 500
            $res = array('error' => '¬нутренн€€ ошибка сервера');
        }
        return $res;
    }
}
            ...
            if( $id != '' && $oldValues["STATUS_ID"] && $oldValues["STATUS_ID"] != $orderStatus && is_array($arSettings['STATUS_BY']) && in_array($orderStatus, $arSettings['STATUS_BY']) && !empty($arSettings['TRIGGERS'])){
                ...
                if(is_array($arSettings['TRIGGERS'])) {
                    if ($trigger_status = array_search($orderStatus, $arSettings['TRIGGERS'])) {
                        ...
                        if (in_array(array_search($oldStatus, $arSettings['STATUS_BY']), array(1, 3, 4, 5, 6,))) $flag_put = false; //echo 'отменен, возврат, транзит, отказ покупател€, отгружен - больше не устанавливаем никакой новый статус';
                        ...
                        if ($flag_put) {
                            if ($trigger_status == 'CLIENT_RECEIVED') {
                                // ищем открытую поставку и добавл€ем в нее заказ
//                            $result = CMaxyssWb::putStatusOrders($id, $status, $arSettings["AUTHORIZATION"]); // устарел
                                ...
                                // ќтмен€ем заказ https://suppliers-api.wildberries.ru/api/v3/orders/{order}/cancel
