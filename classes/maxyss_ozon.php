<?
class CRestQuery{
    public static function rest_query($ClientId = OZON_ID, $ApiKey = OZON_API_KEY, $base_url = OZON_BASE_URL, $data_string, $path){
        $api = new RestClient([
            'base_url' => $base_url,
            'curl_options' => array(
        ...
        if(\Bitrix\Main\Config\Option::get('maxyss.ozon', "LOG_ON",  "N") == "Y") {
            ...
            $eventLog->Add(array("SEVERITY" => 'INFO', "AUDIT_TYPE_ID" => $path, "MODULE_ID" => 'maxyss.ozon', "ITEM_ID" => $ClientId, "DESCRIPTION" => $str_result->response));
        }

        if($str_result->info->content_type != 'text/html') {
...
//                            echo json_encode(array('success' => '/upload/invoice.pdf'));
                ...
                if ($str_result->response != '') {
                    if ($str_result->decode_response()->code == 3) {
                        ...
                        if ($str_result->info->url == "https://api-seller.ozon.ru/v3/posting/fbs/ship") {
            ...
            $res = array('error' => 'text/html ozon no json');
            $eventLog = new \CEventLog;
            $eventLog->Add(array("SEVERITY" => 'INFO', "AUDIT_TYPE_ID" => 'str_result', "MODULE_ID" => 'maxyss.ozon', "ITEM_ID" => $ClientId, "DESCRIPTION" => 'text/html ozon no json'));
            return $res;
        }
    }
}
