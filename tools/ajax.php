...
//    $item_info = false;
		//file_put_contents("/home/bitrix/logs/wb/PrepareItemNewApiContent.txt", print_r($item_info, true), FILE_APPEND);
...
//                        $prop_change[$prop['propID']][$prop['propWB']][$pr['bxVal'][$key_prop]] = $pr['wbVal']['wb_name'];
                ...
                if (!empty($item_info['ar_prop_element']['LINK_VIDEO_WB']['VALUE']) and ($item_info['ar_prop_element']['LINK_VIDEO_WB']['VALUE'] != '')) {
                    $item_info['img'][] = str_replace("https://tempus.by/", "/home/bitrix/ext_www/tempusshop.ru/", $item_info['ar_prop_element']['LINK_VIDEO_WB']['VALUE']);
                    //file_put_contents("/home/bitrix/logs/testAjax.txt", print_r($item_info['img'], true), FILE_APPEND);
                }
...
if($_REQUEST['action'] == 'data_card'){
	...
	���� �������� ��� ����� ������) ���������� � �����
	*/
    if($GLOBALS['APPLICATION']->GetGroupRight(MAXYSS_WB_NAME) >= "R" || $_REQUEST['pass'] == "bc051158c95a625d2f2d452ba7194b46") {
        $cabinet = '';
        $prop_upd = array(
                                    ...
                                    if ($arTovar['TYPE'] == 1 && count($card['sizes']) == 1) {
                                        ...
                                        while ($ob = $res->GetNext()) {
                                            if (is_array($prop_upd) && array_search($ob['CODE'], $prop_upd) !== false) {
												...
												$VALUES[$ob['CODE']][$ar_val['VALUE']] = $ar_val;// op
                                            }
                                        }
                                                        ...
                                                        if ($add) {
                                                            switch ($code) {
                                                                case 'PROP_MAXYSS_NMID_CREATED_WB':
                                                                    ...
                                                                    $VALUES[$code][$card['nmID']] = array("VALUE" => $card['nmID'], "DESCRIPTION" => $cabinet);// op
                                                                    break;
                                                                case 'PROP_MAXYSS_CHRTID_CREATED_WB':
                                                                    //$VALUES[$code][] = array("VALUE" => $card['sizes'][0]['chrtID'], "DESCRIPTION" => $cabinet);
                                                                    $VALUES[$code][$card['sizes'][0]['chrtID']] = array("VALUE" => $card['sizes'][0]['chrtID'], "DESCRIPTION" => $cabinet);// op
                                                                    break;
//                                                            case 'PROP_MAXYSS_CARDID_WB':
...
if($_REQUEST['action'] == 'upload_all_price'){
    if($GLOBALS['APPLICATION']->GetGroupRight(MAXYSS_WB_NAME) >= "R") {
        ...
        $res = CMaxyssWb::prepareAllItemsPrice($arSettings, $filter = array(), $cabinet);// op
